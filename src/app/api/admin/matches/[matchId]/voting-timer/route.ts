...
import { patchVotingTimer } from "@/server/modules/matches/service"
...
export async function PATCH(req: NextRequest, { params }: { params: { matchId: string } }) {
  try {
    ...
    const { deltaSeconds, remainingSeconds } = body
    if (deltaSeconds === undefined && remainingSeconds === undefined) {
      return NextResponse.json<ApiResponse<null>>({ data: null, error: "MISSING_FIELDS" }, { status: 400 })
    }

    const result = await patchVotingTimer(params.matchId, { deltaSeconds, remainingSeconds })
